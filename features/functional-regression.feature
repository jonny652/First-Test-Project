Feature: Manufacturer pages
  As a visitor to NBS Source
  I want to view manufacturer pages
  So that I can find information about a manufacturer

  # Core UI checks per manufacturer: page content, navigation tabs, contact
  # details, and the "add to collection" feature. Each manufacturer is its
  # own Rule so it can have its own Background (a single .feature file may
  # only have one Feature:, but Gherkin's Rule keyword supports per-group
  # Backgrounds within it).

  Rule: Dyson manufacturer page
  ...
  Rule: Abloy UK manufacturer page

    Background:
      Given I am on the NBS Source homepage
      And I close the popup
      When I search for "abloy"
      And I open the manufacturers tab
      And I open the Abloy UK manufacturer page
      Then I should be on the Abloy UK manufacturer page

    Scenario: The page heading is correct
      Then the Abloy heading should be visible
      And the Abloy heading should contain "Abloy UK"

    Scenario: Check the Source logo links back to the homepage
      When i check the Abloy source logo the href is as expected "/en/gb"

    Scenario: Check the "I'm a manufacturer" button is visible with the right text and link
      When i check the Abloy "I'm a manufacturer" button its visible
      Then the Abloy "I'm a manufacturer" button should contain text "I'm a manufacturer"
      Then the Abloy "I'm a manufacturer" button should have the correct href "https://manufacturers.thenbs.com/nbs-source"

    Scenario: I should see the "Back to top" button appear after scrolling down, successfully scrolls the page back to the top when clicked, and then hides itself again
      When I click the "Back to top" button on the Abloy page it behaves as expected

    Scenario:  Assert tabs are all visible, in the correct order, and each href is correct.
      Then the Abloy tabs should be visible, in the correct order, and each href should be correct

    Scenario: Ensure that the "I'm a manufacturer" button contains the correct URL.
      Then Abloy I'm a manufacturer button should have the correct href "https://manufacturers.thenbs.com/nbs-source"

    Scenario: Assert that the Abloy UK telephone number is correct.
      Then Ensure the HREF attribute on the Abloy telephone number is as expected "tel:+44 (0)1902 364500"

    Scenario: Assert that the Abloy UK Website link is visible and the href is correct
      Then Ensure the HREF attribute on the Abloy Website link is as expected "https://www.abloy.co.uk"

    Scenario: Assert the LinkedIn icon is visible and has the expected href
      Then Ensure the HREF attribute on the Abloy LinkedIn icon is as expected "https://www.linkedin.com/company/abloy-uk/"

    Scenario: Ensure that the Heart icon allows logged in users to add an item to their collection
      When I click the Heart icon on the Abloy manufacturer page then it behaves as expected
